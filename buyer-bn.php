...
<body>
    <div align="center">
        <a href="buyer-bn.php?slogid=<?php $ssid=$_GET['slogid']; echo $ssid; ?>&pid=0"> <img src="logo.png" alternate="KineFelun" height="200px" widht="200px"> </a>
    </div>
        ...
        <?php
            ...
            if($con){
                ...
                while($row=mysqli_fetch_row($res)){
                    if($row[0]==$ssid){
                        echo '<div class="text-warning fs-1 text-center border border-bottom shadow p-3 m-3" style="font-weight:bolder;">ক্রেতার তথ্য</div>';
                        echo '<div class="text-info fs-3 text-center border-bottom shadow p-6 m-3" style="font-weight:bolder;" >ক্রেতার আইডি : ' .$row[0]. '</div>';
                        ...
                        echo '<a  style="font-weight:bolder;" class="btn btn-primary d-inline m-2 my-2" href="edit_buyer.php?slogid='.$row[0].'">পরিবর্তন</a>';
                        echo '<a  style="font-weight:bolder;" class="btn btn-secondary d-inline m-2 " href="history.php?slogid='.$row[0].'">অতীত অর্ডার</a>';
        ...
        <div class="container">
            <?php
                ...
                if($check_pid){
                    ...
                    echo '<div class="text-success fs-2 text-center border border-bottom shadow p-3 m-3" style="font-weight:bolder;">প্রোডাক্ট কার্টে যোগ করা হয়েছে</div>';
                }    
            ?>
            <form action="search_product.php?pid=<?php $pid=0; echo $pid; ?>&bid=<?php $bid=$_GET['slogid']; echo $bid; ?>" style="height: 130px; width: 1100px; margin-left:100px ;margin-top:50px; margin-bottom:50px;" method="POST" class='form-control bg-transparent'>
                <input type="text" class='form-control'placeholder='নাম অথবা টাইপ অনুযায়ী সার্চ করুন' style="font-size: 30px" name='nm'>
                <input type="submit" class='form-control btn-primary' value='সার্চ' style="font-size: 30px" name='sub'>
            </form>
        </div>
        <?php
            if($_GET['slogid']){
            ...
            if($con){
                $f=0;
                $res=mysqli_query($con,$q);
                echo '<div class="text-dark fs-1 text-center border-bottom shadow p-4 m-4">আপনার কার্ট :</div>';
                            ...
                            if($row2[0]==$row[3]){
                                ...
                                echo '<div class="card m-4" style="max-width: 18rem; min-width: 16rem" >';
                                    ...
                                    echo '<div class="card-body">';
                                        ...
                                        echo '<p class="card-text" style="font-size: 22px; font-weight: bolder;">টাইপ : '.$row2[2].'</p>';
                                        echo '<p class="card-text" style="font-size: 22px; font-weight: bolder;">দাম : '.$row2[5].'</p>';
                                    echo "</div>";
                                    echo '<div class="card-footer bg-transparent"><a class="btn btn-danger" style="font-size:26px" href="delete_product.php?slogid='.$row[2].'&pid='.$row2[0].'&oid='.$orderid.'&oin='.$row[0].'">ডিলিট</a></div>';
                                echo "</div>";
                ...
                while($orde=mysqli_fetch_row($ordet)){
                    if($orde[1]==$ssid && $orde[4]==0){
                        echo '<div class="text-success fs-1 text-center border border-bottom shadow p-3 m-3" style="font-weight:bolder;">মোট বিল : ' .$orde[2]. '</div>';
                        echo '<div align=center> <a class="btn btn-success" style="font-size:35px;" href="checkout-bn.php?slogid='.$ssid.'">চেক আউট</a> </div>';
                        break;
                    }
                }
